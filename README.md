...
| FPGA utilization | — | LUT 563, LUTRAM 1, FF 1056, DSP 9, IO 43 | — | Vivado implemented design, §8 |
| Maximum frequency | — | ≈ 206 | MHz | 4.854 ns clock period, timing closed |
| Power estimate | — | 0.133 | W | Vivado power report, §8 |
| Verification status | — | **PASS** vs. Python golden model | — | §7 |
| FoM | Throughput / (Power × (LUTs + 50·DSPs + 100·BRAMs)) | ≈ 1,414,438 | (px/s)/(W·resource-unit) | See §9 |
...
Synthesized and implemented in **Vivado**, targeting the **PYNQ-Z2 board (XC7Z020-1CLG400C)**, with a **206 MHz** system clock (4.854 ns period, per `Constraints_cnn_top.xdc`). Figures below are real post-implementation results, not estimates.
...
| FF | 1,056 | 106,400 | 0.99% |
| DSP | 9 | 220 | 4.09% |
| IO | 43 | 125 | 34.40% |

### 8.2 Timing

- Worst Negative Slack (WNS): **0.041 ns** — setup met, 0 failing endpoints out of 1,973
- Worst Hold Slack (WHS): **0.167 ns** — hold met, 0 failing endpoints out of 1,973
- Worst Pulse Width Slack (WPWS): **1.447 ns** — met, 0 failing endpoints out of 1,067
- All user-specified timing constraints are met at the 206 MHz target frequency.

### 8.3 Power

- Total on-chip power: **0.133 W**
- Dynamic power: 0.027 W (20%) — Clocks 0.007 W, Signals 0.009 W, Logic 0.004 W, DSP ≈0.000 W, I/O 0.006 W
- Device static power: 0.106 W (80%)
...
Using real implementation results (LUTs = 563, DSPs = 9, BRAMs = 0, Power = 0.133 W):

- LUT-equivalent denominator = 563 + 50×9 + 100×0 = **1,013**
- Power × denominator = 0.133 × 1,013 ≈ **134.73**
...
- **FOM (spec units) ≈ 6.87×10⁻³** (output px/cycle)/(W·resource-unit)

Equivalently, at the 206 MHz implemented clock: Throughput ≈ (900/973) × 206,000,000 ≈ **190,565,880 output px/s**, giving **FOM ≈ 1,414,438** (output px/s)/(W·resource-unit). Both figures describe the same design — see §10 for which convention to report.
...
| `tb_cnn_top.sv` | Self-checking SystemVerilog testbench |
| `Constraints_cnn_top.xdc` | Pin assignment and 206 MHz clock constraint for the PYNQ-Z2 board |
...
3. Apply `Constraints_cnn_top.xdc` (targets the PYNQ-Z2 board, 206 MHz clock).
